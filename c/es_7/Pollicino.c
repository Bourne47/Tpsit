...
void ordinaDec(Persona classe[], int nRighe){
    ...
    for (sup = nRighe-1; sup > 0; sup--){
        for (Persona *k = classe; k < classe+sup ; k++){
            if (k->nascita.anno > (k+1)->nascita.anno || k->nascita.anno == (k+1)->nascita.anno && k->nascita.mese > (k+1)->nascita.mese || k->nascita.anno == (k+1)->nascita.anno && k->nascita.mese == (k+1)->nascita.mese && k->nascita.giorno > (k+1)->nascita.giorno)
                scambio(k, k+1);
        }
    }
}
...
int main(){
    ...
    char* campo;
    char riga[DIM_RIGA];
    char nomeFile[LUNG] = {"etaPersone.csv"};
